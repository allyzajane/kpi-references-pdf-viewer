...
import { AlertTriangle, Bookmark, ChevronLeft, ChevronRight, Download, ExternalLink, FileText, Image, ListTree, Loader2, Maximize2, Minimize2, PanelLeft, RefreshCw, RotateCw, ScanLine, X, ZoomIn, ZoomOut } from "lucide-react";
...
export default function ContinuousPdfReader({ selectedDocument, access }: { selectedDocument: PortalDocument; access: DocumentAccess }) {
  ...
  const persistReadingState = useCallback((pageNumber: number, pageOffset: number) => {
    if (!pageCount) return;
    saveReadingState(selectedDocument.id, { page: Math.max(1, Math.min(pageCount, pageNumber)), pageOffset: Math.max(0, Math.min(1, pageOffset)), manualZoom, fitMode, rotation });
  }, [fitMode, manualZoom, pageCount, rotation, selectedDocument.id]);
  ...
  useEffect(() => {
    ...
    setPdf(null); setBasePageSize(null); setPage(1); setPageInput("1"); setRotation(0); setFitMode(defaultFit); setManualZoom(1); setOutline([]); setLoading(true); setError(null);
  ...
  }, [fitMode, manualZoom, page, pdf, persistReadingState, rotation]);
