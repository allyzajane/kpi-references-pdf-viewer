...
export default function ContinuousPdfReader({ selectedDocument, access }: { selectedDocument: PortalDocument; access: DocumentAccess }) {
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "f") { event.preventDefault(); event.stopPropagation(); setSearchOpen(true); window.requestAnimationFrame(() => searchInputRef.current?.focus()); }
