...
export default function Home() {
  ...
  return <main className="h-[100dvh] min-h-[100svh] overflow-hidden bg-[#E8F5FC] p-0 text-stone-900 lg:p-4"><div className="mx-auto flex h-full max-w-[1520px] flex-col overflow-hidden bg-[#E8F5FC] shadow-[0_18px_70px_rgba(24,144,207,0.16)] lg:rounded-[1.75rem]"><header className="flex shrink-0 items-center justify-between border-b border-[#BFE4F8] bg-[#E8F5FC] px-4 py-3 sm:px-7 sm:py-4"><div className="flex items-center gap-3"><div className="kpi-key-logo flex size-9 items-center justify-center rounded-xl bg-[#1890CF] shadow-sm" role="img" aria-label={KPI_KEY_TOOLTIP} tabIndex={0}><KpiKeyMark className="size-[21px]" /><span role="tooltip" className="kpi-key-tooltip">{KPI_KEY_TOOLTIP}</span></div><div><p className="font-[family-name:var(--font-display)] text-lg font-bold leading-none tracking-tight text-stone-900">KPI References</p><p className="mt-1 font-[family-name:var(--font-mono)] text-[9px] font-medium uppercase tracking-[0.16em] text-stone-500">Document portal</p></div></div><div className="hidden items-center gap-2 text-xs text-stone-500 sm:flex"><span className="size-2 rounded-full bg-[#44AFE9]" />Auto-refreshes every minute</div></header>{documentsQuery.isLoading ? <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-4 bg-[#E8F5FC] text-stone-600"><Loader2 className="size-9" /><p className="text-sm font-medium">Loading your document collection…</p></div> : listError ? <div className="flex min-h-0 flex-1 items-center justify-center bg-[#E8F5FC] p-6"><div className="max-w-lg rounded-[1.75rem] border border-amber-200 bg-amber-50 p-8 text-center shadow-sm"><div className="mx-auto mb-4 flex size-12 items-center justify-center rounded-full bg-amber-100 text-amber-700"><AlertTriangle className="size-5" /></div><h1 className="font-[family-name:var(--font-display)] text-2xl font-semibold text-stone-900">The collection is unavailable</h1><p className="mt-3 text-sm leading-6 text-stone-600">{listError}</p><Button className="mt-6 rounded-full bg-[#1890CF] px-5 text-white hover:bg-[#187BB3]" onClick={() => void documentsQuery.refetch()}><RefreshCw className="mr-2 size-4" />Retry connection</Button></div></div> : <div className="flex min-h-0 flex-1 flex-col lg:flex-row"><Navigator documents={documents} selectedId={selectedId} onSelect={setSelectedId} search={search} onSearch={setSearch} onRefresh={() => refreshMutation.mutate()} isRefreshing={isRefreshing} isCollapsed={documentsCollapsed} onToggleCollapsed={() => setDocumentsCollapsed(current => !current)} />{viewerArea}</div>}</div></main>;
}
