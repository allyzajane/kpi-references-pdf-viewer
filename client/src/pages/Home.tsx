...
import ContinuousPdfReader from "@/components/ContinuousPdfReader";
import { KpiKeyLoader as Loader2, KpiKeyMark, KPI_KEY_TOOLTIP } from "@/components/KpiKeyBrand";
import { Input } from "@/components/ui/input";
import { useDrivePortal } from "@/hooks/useDrivePortal";
import { cn } from "@/lib/utils";
import { getDocumentsSidebarToggleLabel } from "@/lib/documentSidebar";
...
function Navigator({ documents, selectedId, onSelect, search, onSearch, onRefresh, isRefreshing, isCollapsed, onToggleCollapsed }: { documents: PortalDocument[]; selectedId: string | null; onSelect: (id: string) => void; search: string; onSearch: (value: string) => void; onRefresh: () => void; isRefreshing: boolean; isCollapsed: boolean; onToggleCollapsed: () => void }) {
  ...
  return <aside className={cn("pdf-navigator flex w-full shrink-0 flex-col overflow-hidden border-b border-[#BFE4F8] bg-[#E8F5FC] transition-[width] duration-200 motion-reduce:transition-none lg:border-b-0 lg:border-r", isCollapsed ? "lg:w-[72px]" : "lg:w-[348px]")} aria-label="Documents sidebar">
    ...
    <a href="/setup" className={cn("mx-4 mb-4 rounded-xl border border-[#BFE4F8] bg-white px-3 py-2 text-center text-xs font-semibold text-[#1890CF] transition-colors hover:bg-[#E8F5FC] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#44AFE9] lg:mx-5", isCollapsed && "lg:hidden")}>Drive connection status</a>
    <div className={cn("hidden h-full flex-col items-center gap-4 border-[#BFE4F8] bg-[#E8F5FC] py-4 lg:flex", !isCollapsed && "lg:hidden")}>
      <Button variant="outline" size="icon" className="size-10 rounded-full border-[#96D2F3] bg-white text-[#1890CF] hover:bg-[#E8F5FC]" onClick={onToggleCollapsed} aria-label={getDocumentsSidebarToggleLabel(true)} title={getDocumentsSidebarToggleLabel(true)}><PanelLeftOpen className="size-4" /></Button>
      <div className="flex size-9 items-center justify-center rounded-xl bg-[#1890CF]" aria-hidden="true"><KpiKeyMark className="size-5" /></div>
      <span className="font-[family-name:var(--font-mono)] text-[9px] font-semibold tracking-wide text-stone-500 [writing-mode:vertical-rl]">{documents.length} FILES</span>
    </div>
  </aside>;
}
...
export default function Home() {
  ...
  const { documents, documentsLoading, documentsError, isRefreshing, refreshDocuments, access, accessLoading, accessError, refreshAccess } = useDrivePortal(selectedId);
  const selectedDocument = documents.find(item => item.id === selectedId) ?? null;

  useEffect(() => {
    document.title = documentsLoading ? "Opening KPI References..." : "KPI References";
  }, [documentsLoading]);
  ...
  useEffect(() => {
    if (!selectedDocument) {
      if (!documentsLoading) window.localStorage.removeItem(LAST_DOCUMENT_KEY);
      return;
    }
    window.localStorage.setItem(LAST_DOCUMENT_KEY, selectedDocument.id);
  }, [documentsLoading, selectedDocument]);

  const listError = documentsError;
  const viewerArea = !selectedDocument ? <section className="flex min-h-0 flex-1 items-center justify-center bg-[#eceae4] p-7"><div className="max-w-md text-center"><div className="mx-auto mb-5 flex size-16 items-center justify-center rounded-full bg-white text-[#355d54] shadow-sm"><BookOpen className="size-7" /></div><h1 className="font-[family-name:var(--font-display)] text-3xl font-semibold text-stone-900">Your reading room awaits</h1><p className="mt-3 text-sm leading-6 text-stone-600">Select a PDF from the collection to view it here without leaving the portal.</p></div></section> : accessLoading ? <section className="flex min-h-0 flex-1 flex-col items-center justify-center gap-3 bg-[#eceae4] text-stone-600"><Loader2 className="size-7 animate-spin text-[#355d54]" /><p className="text-sm font-medium">Securing document access…</p></section> : accessError || !access ? <section className="flex min-h-0 flex-1 items-center justify-center bg-[#eceae4] p-7"><div className="max-w-md rounded-[1.5rem] border border-amber-200 bg-amber-50 p-7 text-center shadow-sm"><div className="mx-auto mb-4 flex size-11 items-center justify-center rounded-full bg-amber-100 text-amber-700"><AlertTriangle className="size-5" /></div><h2 className="font-[family-name:var(--font-display)] text-xl font-semibold text-stone-900">Document unavailable</h2><p className="mt-2 text-sm leading-6 text-stone-600">{accessError || "The portal could not verify access to this document."}</p><Button variant="outline" className="mt-5 rounded-full border-stone-300 bg-white" onClick={() => selectedId && void refreshAccess(selectedId)}>Try again</Button></div></section> : <ContinuousPdfReader key={selectedDocument.id} selectedDocument={selectedDocument} access={access} />;

  return <main className="h-[100dvh] min-h-[100svh] overflow-hidden bg-[#E8F5FC] p-0 text-stone-900 lg:p-4"><div className="mx-auto flex h-full max-w-[1520px] flex-col overflow-hidden bg-[#E8F5FC] shadow-[0_18px_70px_rgba(24,144,207,0.16)] lg:rounded-[1.75rem]"><header className="flex shrink-0 items-center justify-between border-b border-[#BFE4F8] bg-[#E8F5FC] px-4 py-3 sm:px-7 sm:py-4"><div className="flex items-center gap-3"><div className="kpi-key-logo flex size-9 items-center justify-center rounded-xl bg-[#1890CF] shadow-sm" role="img" aria-label={KPI_KEY_TOOLTIP} tabIndex={0}><KpiKeyMark className="size-[21px]" /><span role="tooltip" className="kpi-key-tooltip">{KPI_KEY_TOOLTIP}</span></div><div><p className="font-[family-name:var(--font-display)] text-lg font-bold leading-none tracking-tight text-stone-900">KPI References</p><p className="mt-1 font-[family-name:var(--font-mono)] text-[9px] font-medium uppercase tracking-[0.16em] text-stone-500">Document portal</p></div></div><div className="hidden items-center gap-2 text-xs text-stone-500 sm:flex"><span className="size-2 rounded-full bg-[#44AFE9]" />Auto-refreshes every minute</div></header>{documentsLoading ? <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-4 bg-[#E8F5FC] text-stone-600"><Loader2 className="size-9" /><p className="text-sm font-medium">Loading your document collection…</p></div> : listError ? <div className="flex min-h-0 flex-1 items-center justify-center bg-[#E8F5FC] p-6"><div className="max-w-lg rounded-[1.75rem] border border-amber-200 bg-amber-50 p-8 text-center shadow-sm"><div className="mx-auto mb-4 flex size-12 items-center justify-center rounded-full bg-amber-100 text-amber-700"><AlertTriangle className="size-5" /></div><h1 className="font-[family-name:var(--font-display)] text-2xl font-semibold text-stone-900">The collection is unavailable</h1><p className="mt-3 text-sm leading-6 text-stone-600">{listError}</p><Button className="mt-6 rounded-full bg-[#1890CF] px-5 text-white hover:bg-[#187BB3]" onClick={() => void refreshDocuments()}><RefreshCw className="mr-2 size-4" />Retry connection</Button></div></div> : <div className="flex min-h-0 flex-1 flex-col lg:flex-row"><Navigator documents={documents} selectedId={selectedId} onSelect={setSelectedId} search={search} onSearch={setSearch} onRefresh={() => void refreshDocuments()} isRefreshing={isRefreshing} isCollapsed={documentsCollapsed} onToggleCollapsed={() => setDocumentsCollapsed(current => !current)} />{viewerArea}</div>}</div></main>;
}
