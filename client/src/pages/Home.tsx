...
function KpiKeyMark({ className }: { className?: string }) {
  return <svg viewBox="0 0 32 32" fill="none" className={className} aria-hidden="true">
    ...
    <path d="M13.8 15.8L27.2 15.8V19.1H24.4V22H21.4V19.1H18.7V17.8H13.8V15.8Z" fill="#F7CE62" stroke="#FFF3C4" strokeLinejoin="round" strokeWidth="1.1" />
    <path d="M6.75 14.1V11.9M9 14.1V9.1M11.25 14.1V10.35" stroke="#355D54" strokeLinecap="round" strokeWidth="1.45" />
  </svg>;
}
...
function Navigator({ documents, selectedId, onSelect, search, onSearch, onRefresh, isRefreshing, isCollapsed, onToggleCollapsed }: { documents: PortalDocument[]; selectedId: string | null; onSelect: (id: string) => void; search: string; onSearch: (value: string) => void; onRefresh: () => void; isRefreshing: boolean; isCollapsed: boolean; onToggleCollapsed: () => void }) {
  ...
  return <aside className={cn("pdf-navigator flex w-full shrink-0 flex-col overflow-hidden border-b border-stone-200 bg-[#fbfaf7] transition-[width] duration-200 motion-reduce:transition-none lg:border-b-0 lg:border-r", isCollapsed ? "lg:w-[72px]" : "lg:w-[348px]")} aria-label="Documents sidebar">
    ...
    <div className={cn("hidden h-full flex-col items-center gap-4 border-stone-200 bg-[#fbfaf7] py-4 lg:flex", !isCollapsed && "lg:hidden")}>
      <Button variant="outline" size="icon" className="size-10 rounded-full border-stone-300 bg-white text-stone-600 hover:bg-stone-100" onClick={onToggleCollapsed} aria-label={getDocumentsSidebarToggleLabel(true)} title={getDocumentsSidebarToggleLabel(true)}><PanelLeftOpen className="size-4" /></Button>
      <div className="flex size-9 items-center justify-center rounded-xl bg-[#244b42]" aria-hidden="true"><KpiKeyMark className="size-5" /></div>
