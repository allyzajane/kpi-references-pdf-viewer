...
The Drive status endpoint now requires a server-side `DRIVE_STATUS_ACCESS_TOKEN` and returns `401` before any status data when no matching operator token is supplied. The compact `/setup` screen now presents a password-style operator-token field, does not request or display Google Drive credential values, and does not retain the token in browser storage. Regression coverage verifies both unauthorized rejection and an accepted installed token, while type checks, production build, and Pages Functions compilation all pass.
